...
export class UserService {
  ...
  checkIdValid(id: string): Observable<any> {
    return this.http
      .get<User>(`${this.URL}/id-check/${id}`)
      .pipe(catchError(this.handleError('Id validation')));
  }
  ...
  login(userId: string, password: string): Observable<User> {
    return this.http
      .post<User>(`${this.URL}/login`, { userId, password })
  ...
  getUserData(token: string): Observable<User> {
    return this.http
      ...
      .pipe(catchError(this.handleError<User>('Get user data')));
  }
