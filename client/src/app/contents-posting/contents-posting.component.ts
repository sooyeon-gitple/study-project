...
export class ContentsPostingComponent implements OnInit {
  ...
  */
      ...
      this.userService.getUserData(localUserToken).subscribe(
        userData => { 
          if(userData._id){
            ...
            window.alert("로그인 시간이 만료되었거나 정상적인 사용자가 아닙니다.");
  ...
  editContent():void{
    this.contentService.editContent(this.token,this.postingModel).subscribe(
      result =>{
        if(result?._id){
  ...
  deleteContent():void{
    if(window.confirm('삭제하시겠습니까?') ){
      this.contentService.deleteContent(this.token,this.id).subscribe(
        res =>{
